...
#define SYMBOLIC_MEM_BASE 0x10001000   // Default symbolic memory base
...
static long prepare_argument(syscall_context_t *ctx, const char *json_str, 
    ...
    if (is_result_reference(json_str, token)) {
        ...
        if (idx >= 0 && idx < ctx->result_count) {
            long saved_val = ctx->saved_results[idx];
            printf("  Using saved result $%d: %ld (0x%lx)\n", idx, saved_val, saved_val);
            
            // In symbolic mode, we need to allocate memory and store the value
            // so it can be dereferenced later for integer types
            if (ctx->symbolic_mode) {
                switch (arg_type) {
                    case ARG_CHAR: {
                        char *val_ptr = (char*)symbolic_alloc(ctx, sizeof(char));
                        if (!val_ptr) return 0;
                        *val_ptr = (char)saved_val;
                        return (long)val_ptr;
                    }
                    case ARG_SHORT: {
                        short *val_ptr = (short*)symbolic_alloc(ctx, sizeof(short));
                        if (!val_ptr) return 0;
                        *val_ptr = (short)saved_val;
                        return (long)val_ptr;
                    }
                    case ARG_INT:
                    case ARG_FD: {
                    ...
                    default:
                        // For pointers and strings, return the value directly
                        return saved_val;
                }
            } else {
                // Non-symbolic mode: return the value directly
                return saved_val;
            }
        } else {
