...
#define MAX_SAVED_RESULTS 32  // Maximum number of saved syscall results
...
int parse_syscall_input(const char *json_str, syscall_context_t *ctx) {
    ...
    // First pass: parse syscall number and symbolic mode before processing args
    for (int i = 1; i < r; i++) {
    ...
    // Second pass: parse arguments with correct symbolic mode
    for (int i = 1; i < r; i++) {
